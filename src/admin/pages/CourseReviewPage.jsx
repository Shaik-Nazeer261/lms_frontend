  ...
  const fetchCourses = async () => {
    try {
      const res = await api.get(
        `/api/admin/pending-courses/?page=${page}&sort=${sort}&search=${search}`
      );
        ...
        {filteredCourses.map((course) => (
          ...
          >
            <div className="flex items-center gap-4">
              <img
                src={`${import.meta.env.VITE_BACKEND_URL}/api${course.course_image}`}
                alt={course.title}
              ...
              <div>
                ...
                <p className="text-sm text-gray-500">
                  Course by {course.instructor || "Unknown"}
                </p>
              </div>
            </div>
