...
const AdminDashboard = () => {
  ...
  const fetchActiveCourses = async () => {
    try {
      ...
      const active = res.data.filter(
        (course) => course.creation_progress === 100
      );
  ...
  const handleAddInstructor = async (e) => {
    ...
    try {
      ...
      setMessage({
        type: "success",
        text: response.data.message || "Instructor added successfully!",
      });
                ...
                <tbody>
                  {instructors.map((inst, idx) => (
                    <tr key={inst.id || idx}>
                      ...
                      {/* <td className="px-3 py-2 border">{inst.first_name} {inst.last_name}</td> */}
                ...
                {pendingCourses.map((course, idx) => (
                  <tr key={course.id}>
                    ...
                    <td className="px-3 py-2 border">
                      <button
                        onClick={() =>
                          navigate(`/admin/admincourseview/${course.id}`)
                        }
