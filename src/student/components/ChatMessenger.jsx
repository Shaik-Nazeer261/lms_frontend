...
export default function ChatMessenger() {
  ...
  const handleEditSubmit = (id) => {
    ...
    api
      .put(`/api/chat/private/message/${id}/edit/`, { message: editedMessage })
      .then(() => {
        setChats(
          chats.map((m) =>
            m.id === id ? { ...m, message: editedMessage, is_edited: true } : m
          )
        );
        cancelEdit();
      })
  ...
  const deleteMessage = (id) => {
    ...
    api
      .delete(`/api/chat/private/message/${id}/delete/`)
  ...
  useEffect(() => {
    api
      .get("/api/chat/private/instructors/")
  ...
  useEffect(() => {
    ...
    api
      .get(
        `/api/chat/private/thread/${selectedContact.instructor_id}/?course_id=${selectedContact.course_id}`
      )
      .then((res) => {
        setChats(res.data);
  ...
  const sendMessage = () => {
    ...
    api
      .post(
        `/api/chat/private/thread/${selectedContact.instructor_id}/`,
        formData
      )
      .then((res) => {
          ...
          {contacts.map((user) => (
            <div
              ...
              className={classNames(
                ...
                selectedContact?.instructor_id === user.instructor_id &&
                  "bg-blue-50"
              )}
        ...
        {selectedContact ? (
          <>
            ...
            <div className="flex items-center justify-between p-4 border-b bg-white">
              <div className="flex items-center gap-3">
                <img
                  ...
                  className="w-10 h-10 rounded-full"
                  alt={selectedContact.instructor_name}
                />
                <div>
                  <h4 className="font-semibold">
                    {selectedContact.instructor_name}
                  </h4>
            ...
            <div className="flex-1 p-6 overflow-y-auto space-y-4">
              ...
              {chats.map((msg) => {
                const isMe =
                  msg.sender_name !== selectedContact.instructor_name;
                return (
                  <div
                    key={msg.id}
                    className={`flex ${
      ...
      {showComposeModal && (
        <div className="fixed inset-0 bg-[#000000B2] z-50 flex items-center justify-center">
          <div className="bg-white  shadow-lg w-[500px] p-6 relative">
            ...
            <h2 className="text-lg font-semibold text-[#1b1c57] mb-4">
              New Message
            </h2>
            ...
            <div className="flex justify-between">
              ...
              <button
                onClick={() => {
                  ...
                  api
                    .post(
                      `/api/courses/${courseId}/chat/private/${instructorId}/`,
                      formData
                    )
                    .then(() => {
