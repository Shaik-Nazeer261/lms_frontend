...
const [feedback, setFeedback] = useState("");
  ...
  useEffect(() => {
    const fetchCourse = async () => {
      try {
        const res = await api.get(`/api/watch-course/${id}/`);
        const data = res.data;
        setCourse(data);
    ...
    <div className="bg-white text-[#00113D]">
      <div className="flex justify-between bg-[#F6F9FC] p-4">
        ...
        <div className="flex gap-4">
          <button
  className="bg-white text-blue-600 border px-4 py-2 rounded shadow-sm hover:bg-gray-100"
  onClick={() => setShowReviewPopup(true)}
>
  Write A Review
</button>

          <button className="bg-blue-500 text-white px-4 text-sm shadow-md hover:bg-blue-600">
            Next Lecture
          </button>
        </div>
      </div>
                ...
                <div className="mt-6  pt-4">
                  ...
                  <div className="flex items-center gap-10">
                    ...
                    <div>
                      ...
                      <p className="text-sm text-gray-600">
                        ⭐ {course.rating} Course Rating • 👥 {course.enrolled}{" "}
                        Students • 📚 {course.instructor.courses} Courses
                      </p>
                      <p className="text-sm text-gray-600">{course.instructor.biography}</p>
                    </div>
                  </div>
                </div>
              )}
          ...
          <div className="bg-white p-4 rounded-md shadow w-full">
            <div className="flex justify-between items-center text-sm mb-3">
              ...
              <span className="text-green-600 font-medium ">
                {course.completedPercent || 0}% Completed
              </span>
            </div>
            <div className="w-full bg-gray-200 h-1 rounded-full ">
              <div
                className="bg-green-500 h-1 rounded-full"
                style={{ width: `${course.completedPercent || 0}%` }}
              ></div>
            </div>
            <p className="text-center text-sm text-red-600 font-semibold mt-1">
              Valid upto -{" "}
              <span className="font-bold">{course.validTill || "N/A"}</span>
            </p>
                          ...
                          >
                            <div className="flex items-center gap-2 ">
                              <img
                                src={
                                  lecture.completed
                                    ? checked // ✅ When completed
                                    : lecture.is_active
                                    ? select // 🟦 When currently active
                                    : unchecked // ⬜ When not completed
                                }
